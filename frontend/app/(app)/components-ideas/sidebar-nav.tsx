...
function RailButton({
  ...
  return (
    ...
    >
      ...
      {count && (
        <span className="sidebar-copy mr-2 shrink-0 text-[12px] font-medium tabular-nums text-ink-3">{count}</span>
      )}
    </button>
  );
}

function WorkspaceMenu({ position, onClose }: { position: { top: number; left: number }; onClose: () => void }) {
  return createPortal(
    <div
      data-workspace-menu
